...
export default function ProjectCard({ project }: { project: any }) {
  ...
  const template = project.template || "1";
  const title = project.title || "Proje Adı";
  const description = project.description || project.desc || "";
  const image = project.image || project.apk || "";
  const url = project.url || project.demo || "";

  if (template === "2") {
    return (
      <div className="group relative w-full h-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
        {image && (
  ...
  if (template === "3") {
    return (
      <div className="group w-full h-full border-l-4 border-slate-900 dark:border-white pl-6 py-2 hover:border-pink-500 transition-colors">
  ...
  if (template === "4") {
    return (
      <div className="flex flex-col md:flex-row w-full h-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
        {image && (
  ...
  if (template === "5") {
    return (
      <div className="relative w-full h-full p-10 rounded-3xl overflow-hidden shadow-2xl border border-white/20 dark:border-slate-800/50 flex flex-col items-center text-center">
  ...
  return (
    <div className="group relative w-full h-full bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 hover:border-blue-500/50 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-2 overflow-hidden">
